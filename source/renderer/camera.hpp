...
public:
     ...
     *
     * Устанавливает новую позицию камеры в сцене, не меняя направление взгляда
     *
    ...
    /**
     * @brief Добавление угла рыскания
     *
     * Добавляет переданный в градусах угол рыскания к текущему
     *
     * @param[in] yaw_offset Добавление угла рыскания
     */
    void AddYaw(const float yaw_offset);

    /**
     * @brief Добавления угла тангажа
     *
     * Добавляет переданый в градусах угол тангажа к текущему. При выходе результата за диапазон
     * [-90.0, 90.0] отсекает его по ближайщей границе
     *
     * @param[in] pitch_offset Добавление угла тангажа
     */
    void AddPitch(const float pitch_offset);

    /**
     * @brief Сдвиг камеры
     *
     * Сдвигает позицию камеру на переданный вектор, не меняя направления взгляда
     *
     * @param[in] translate_vector Вектор сдвига
     */
    void Translate(const Vector& translate_vector);

    /**
     * @brief Перемещение камеры вперед/назад
     *
     * Перемещает камеру вдоль направления взгляда на расстояние distance с учeтом знака, то есть
     * положительное значение соответствует перемещению вперед, а отрицательное - назад
     *
     * @param[in] distance Расстояние
     */
    void MoveForward(const float distance);

    /**
     * @brief Перемещение камеры вправо/влево
     *
     * Перемещает камеру вправо относительно направления взгляда на расстояние distance с учетом
     * знака, то есть положительное значение соответствует перемещению вправо, а отрицательное -
     * влево
     *
     * @param[in] distance Расстояние
     */
    ...
    /**
     * @brief Перемещение камеры вверх/вниз
     *
     * Перемещает камеру вверх относительно направления взгляда на расстояние distance с учетом
     * знака, то есть положительное значение соответствует перемещению вверх, а отрицательное -
     * вниз
     *
     * @note Функция перемещает камеру вдоль текущего направления вверх с учетом наклона камеры. Для
     * перемещения относительно направления вверх сцены следует использовать MoveWorldUp
     *
     * @param[in] distance Расстояние
     */
     ...
     * @brief Перемещение камеры вверх/вниз
     *
     * Перемещает камеру вверх относительно вертикальной оси сцены на расстояние distance с учетом
     * знака, то есть положительное значение соответствует перемещению вверх, а отрицательное -
     * вниз
     *
     * @note Функция перемещает камеру вдоль направления вверх сцены. Для
     * перемещения относительно текущего направления вверх с учетом наклона камеры следует
     * использовать MoveUp
     *
     * @param[in] distance Расстояние
     */
    void MoveWorldUp(const float distance);
